...
export function useVoice(activeWs) {
  ...
  const remoteAnalyserCtxRef = useRef(null)
  ...
  async function initiatePeerConnection(peerId, peerUser, isPolite) {
    ...
    // Adiciona tracks de áudio local e aplica configuração otimizada para voz.
    if (localAudioStreamRef.current) {
      localAudioStreamRef.current.getAudioTracks().forEach(t => {
        const sender = pc.addTrack(t, localAudioStreamRef.current)
    ...
    startPeerStatsMonitor(peerId, pc)
  }
  ...
  async function configureAudioSender(sender) {
    try {
      ...
      if ('priority' in params) params.priority = 'high'

      await sender.setParameters(params)
    } catch (err) {
      console.warn('Configuração do áudio não suportada:', err)
    }
  }

  function startPeerStatsMonitor(peerId, pc) {
    if (statsTimersRef.current.has(peerId)) return
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (inboundAudio) {
          console.debug(`[Talky] Áudio ${peerId.slice(0, 8)}`, {
        ...
        if (candidatePair?.currentRoundTripTime != null) {
          console.debug(`[Talky] RTT ${peerId.slice(0, 8)}: ${Math.round(candidatePair.currentRoundTripTime * 1000)}ms`)
        }
      } catch (err) {
        console.debug('Stats WebRTC indisponíveis:', err)
      }
    }, 5000)

    statsTimersRef.current.set(peerId, timer)
  }

  function stopPeerStatsMonitor(peerId) {
    const timer = statsTimersRef.current.get(peerId)
    if (timer) clearInterval(timer)
    statsTimersRef.current.delete(peerId)
  }
  ...
  function closePeerConnection(peerId) {
    ...
    stopPeerStatsMonitor(peerId)
    const remoteAnalyser = remoteAnalysersRef.current.get(peerId)
    if (remoteAnalyser) {
      try { remoteAnalyser.source.disconnect() } catch {}
      try { remoteAnalyser.analyser.disconnect() } catch {}
  ...
  function setupRemoteAudioAnalyser(peerId, remoteStream) {
    try {
      ...
      if (!remoteAnalyserCtxRef.current) {
        remoteAnalyserCtxRef.current = new AC()
      }
      const ctx = remoteAnalyserCtxRef.current
      const existing = remoteAnalysersRef.current.get(peerId)
      if (existing?.stream === remoteStream) return
      if (existing) {
      ...
      const data = new Uint8Array(analyser.frequencyBinCount)
      remoteAnalysersRef.current.set(peerId, { stream: remoteStream, source, analyser })

      function check() {
        if (!remoteAudiosRef.current.has(peerId) || remoteAnalysersRef.current.get(peerId)?.stream !== remoteStream) {
          return
        }
        analyser.getByteFrequencyData(data)
        const avg = data.reduce((a, b) => a + b, 0) / data.length
  ...
  const leaveVoiceRoom = useCallback(async () => {
    ...
    if (remoteAnalyserCtxRef.current && remoteAnalyserCtxRef.current.state !== 'closed') {
  ...
  const startScreenShare = useCallback(async () => {
    ...
    try {
      ...
      // No Electron, o picker informa a qualidade escolhida.
      // Em navegador normal essa API não existe e usamos o comportamento padrão.
      let shareSelection = null
      try {
        shareSelection = await window.talkyDesktop?.getLastScreenShareSelection?.()
      } catch (selectionError) {
        console.debug('Seleção de compartilhamento não disponível:', selectionError)
      }
      ...
      const qualityProfile = qualityProfiles[shareSelection?.quality] || qualityProfiles['1080p30']
      const videoTrack = stream.getVideoTracks()[0]

      if (videoTrack) {
        try {
          await videoTrack.applyConstraints({
            width: { ideal: qualityProfile.width, max: qualityProfile.width },
            height: { ideal: qualityProfile.height, max: qualityProfile.height },
            frameRate: { ideal: qualityProfile.frameRate, max: qualityProfile.frameRate },
          })
        } catch (constraintError) {
          console.warn('Não foi possível aplicar a qualidade escolhida; usando a qualidade disponível:', constraintError)
        }

        // Ajuda o navegador a priorizar detalhes/texto durante screen share.
        try {
